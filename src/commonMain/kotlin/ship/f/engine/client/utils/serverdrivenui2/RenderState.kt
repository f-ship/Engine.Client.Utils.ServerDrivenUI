...
fun Render(
    ...
    client.apply {
        if (state.id.isAutoGenerated || state.id.ignoreReactive) {
            RenderStatic(
                state = state,
                modifier = modifier
            )
...
fun RenderDynamic(
    ...
    client.apply {
        when (state) {
            ...
            is LazyGridState2 -> LazyGrid2(s = getReactiveState(state.id), m = modifier)
            ...
            is CameraGalleryState2 -> CameraGallery2(s = getReactiveState(state.id), m = modifier)
        }
    }
}
